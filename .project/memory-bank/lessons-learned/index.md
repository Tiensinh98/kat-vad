...
## 35. [HIGH] Experiments — a pre-registered threshold needs a MEASURED lever, not an assumed one
**Triggers:** remedy, lever, knob, if the gate fails, gate W, clip oracle, window-max-per-clip, window-length, risk section, runbook advice, plan prose, C32, C33, DADA original, T2
**Problem:** [[lesson-33]] fixed *whether a threshold can be met*; it says nothing about **which flag moves it**, and a plan that names the wrong one costs a full rebuild cycle to discover. Phase 2's risk 3 named `--window-max-per-clip` as the knob to reach for if Gate W drifted and warned *against* `--window-length` (it re-fires [[lesson-32]] retention). Gate W then failed on exactly one criterion — clip oracle **0.7529** against a 0.75 bar — and the named remedy is **inert** there: measured on the real archive census, caps 2/3/4/6 at W=16 give 0.7765 / 0.7527 / 0.7529 / 0.7580, a **0.025 span across a 3x range**, while the window length spans **0.61 → 0.75**. The reason is in C33's own closed form: the oracle follows the abnormal share of test *frames*, and the cap thins both classes at once, so it cancels. The plan also printed a predicted oracle of **0.6631** at W=16 that belongs to a W of about 24 — which fails retention (0.899) and class ratio (3.87) instead. Following the prose would have produced three useless rebuilds; sweeping both flags against the real census found **W=20 / hop 8** (oracle 0.7037, retention 0.957, 798 two-class windows, all six criteria pass) in one pass, on CPU, with no re-extraction, because the feature cache is keyed by source clip and a window is a slice.
**Bad:** `if the gate drifts, move --window-max-per-clip; do not touch --window-length` — a remedy asserted from mechanism intuition, beside a threshold whose predicted value came from a simulation over the annotation alone.
**Good:** sweep every candidate flag against the **real** census and publish the trade-off table beside the threshold, so the remedy is a measured column and not a sentence. Each row costs a label rebuild (seconds) because windowing never touches the feature cache.
**Rule:** Publish a measured lever table beside every pre-registered threshold — one row per candidate flag, on the real data — and never write a remedy sentence that has not been measured.
**Enforcement (2026-09-16):** `colab/DADA2000Origin/phase_2.ipynb` §5 carries a per-criterion `REMEDY` map naming the measured lever; the T2 geometry table is inlined at `core/constants.py:DADA_ORIGIN_WINDOW_LENGTH`; `.project/plans/katvad-dada-original-phase2-t2.md` §6.1 holds the curve and §7 risk 3 is struck through with the correction.
**Files:** .project/plans/katvad-dada-original-phase2-t2.md §6.1 + §7, core/constants.py:DADA_ORIGIN_WINDOW_LENGTH, colab/DADA2000Origin/phase_2.ipynb, core/eda/protocol.py:clip_constant_oracle

## 36. [HIGH] Tooling — a resume API is not a load API; a read-only tool that calls it inherits contracts it cannot satisfy
**Triggers:** grad_probe, load_checkpoint, warm_start_model, --checkpoint, --init-weights, resume, probe, diagnostic, read-only tool, stage-1 checkpoint, clip_text_model, load_clip, Missing key(s) in state_dict, D2
**Problem:** `Trainer.load_checkpoint` is the **same-run resume** path: a strict `load_state_dict` plus an optimizer / scheduler / scaler / RNG restore. `core/tools/grad_probe.py` (the D2 instrument) called it for its `--checkpoint` flag, because that is the method whose name reads like "load a checkpoint". But stage 1 builds the model with `load_clip=False` (`core/train.py:build_trainer` — stage 1 trains only `kip.*` and needs no text conditioning), so a stage-1 checkpoint carries **zero** `clip_text_model.*` keys while the stage-2 model the probe builds carries **197**. The probe's pre-registered **D2-c** arm — the stage-1 end point, probed under the stage-2 objective, which is the state stage 2 actually started from — therefore died with `RuntimeError: Missing key(s) in state_dict` *after* staging every cache, building the dataset and downloading CLIP. The optimizer restore behind it would have failed next anyway: a stage-1 optimizer holds only the KIP parameter group. The tolerance for that one absence already existed one function away, in `warm_start_model` (the stage-1 → stage-2 path, which skips exactly the `clip_text_model.` prefix and restores nothing else); the new tool simply did not call it. Nothing caught it: `ruff`/`mypy`/`bandit` clean, and the probe's 10 tests never passed `--checkpoint` at all, so the only flag that can crash it was the one flag with no coverage.
**Bad:** `trainer.load_checkpoint(args.checkpoint)  # in a read-only diagnostic`
**Good:** `warm_start_model(trainer.model, args.checkpoint, flag="--checkpoint")`
**Rule:** Load weights into a read-only tool through the warm-start path, never the resume path, and give every checkpoint-taking CLI flag a test that actually passes one.
**Enforcement (2026-09-20):** `core/tests/test_grad_probe.py::TestCli` gained two tests — a checkpoint with no text tower loads (a stand-in module reproduces the stage-1 shape with no CLIP download), and a checkpoint missing a `kip.*` key raises `ValueError` naming `--checkpoint`. Both fail on the pre-fix code, the first with the exact `Missing key(s)` trace. `warm_start_model` took a keyword-only `flag` so its error names the option the caller typed.
**Files:** core/tools/grad_probe.py:main, core/train.py:warm_start_model, core/train.py:build_trainer, core/tests/test_grad_probe.py::TestCli

## 37. [HIGH] Losses — a raw loss value measures its target's units, not the fit; weight it against a constant-predictor baseline
**Triggers:** lambda_rec, kip_rec, L_KIP_rec, MSE, regression loss, auxiliary head, loss weight, normalize target, e_O, flow target, unnormalized, loss share, gradient ratio, trunk capture, D1, D2, grad_probe, mag_max, R2, constant predictor
**Problem:** `L_KIP_rec` is a bare masked MSE (`core/kip/losses.py:30-42`) against `e_O`, and `e_O` is **23 unnormalized frame-global RAFT scalars** — `mag_mean/std/max`, `u/v mean/std` in **raw pixel units**, plus an L1-normalized 16-bin angle histogram — lifted to 256-d by a fixed Gaussian map. Nothing normalizes them between `core/flow/raft_extract.py:86-118` and `core/data/dataset.py:114`. Every other term in the objective is a BCE or an InfoNCE and is **O(1) by construction**, so at `lambda_rec = 1.0` the two were never on a comparable scale: measured on T2, `kip_rec` is **92.6–93.0 %** of `total`, and one raw stat (`mag_max`, mean 27.19) carries **83.0 %** of the target's `E[s²]`. The reading "the PMG head fits badly" is simply unavailable — a constant global-mean predictor scores **MSE 31.64** on the same target, so the measured 11.62 is **R² = 0.633**, a good fit that *looks* like a catastrophic loss. And the scale is not inert: `PMGFlowHead` reads `v^t` and stage 2 freezes nothing (`core/train.py:191-195` freezes only in stage 1), so at the shared temporal encoder `|g_KIP| / |g_task|` = **3.1** (11.6 at the stage-1 end) with `cos(g_KIP, g_task)` = **−0.001** — KIP **captures the trunk orthogonally**, spending capacity rather than fighting for it. Every task loss ends **24–32 %** higher with KIP on, and T2 micro AUC drops **0.0129** (3/3 seeds, t95 excluding zero). Nothing raises: the run completes, the losses fall, and the number looks like a loss.
**Bad:** weighting a regression term at 1.0 beside classification terms, and reading its raw value as fit quality.
```yaml
loss:
  lambda_rec: 1.0        # against a target whose constant-predictor MSE is unmeasured
```
**Good:** measure the baselines first, report R² against them, and derive the weight.
```bash
python -m core.tools.eda report --sections features \
  --flow-dir "$CACHE/flow/v1/$DATASET" ...   # prints Z / V / W baselines (EDA §4.3.1)
# R^2 = 1 - kip_rec / V ;  equalizing weight = 1/V ;  then confirm at the trunk:
python -m core.tools.grad_probe --checkpoint ... --num-batches 8   # rho, cos
```
...
**Outcome (2026-09-26, Option A / `flow/v2_zscore`):** the rule worked as predicted. `V_v2` = 1.0047 → `lambda_rec` 0.9953. `rho` at stage-2 end went **3.105 → 0.179**. The cost went too: KIP-on v2 − v1 on T2 micro **+0.0204 [+0.0010, +0.0398]**. Against KIP-off it is **+0.0076 [−0.0002, +0.0154]**, a bounded null. Caveat: on the standardized target `kip_rec` 0.72 > the item-mean oracle 0.654, so v1's `K < W` pass was carried by `mag_max`. **Re-check every oracle comparison after rescaling a target.** Record: `core/docs/RESULTS_DADA_ORIG_T2.md`.
**Files:** core/kip/losses.py:30-42, core/flow/raft_extract.py:86-118, core/data/dataset.py:114, core/train.py:191-195, core/eda/features.py:flow_stats, core/tools/grad_probe.py, outputs/v1/DADA2000_orig_diag_kip_loss_scale/, core/docs/RESULTS_DADA_ORIG_T2.md

## 38. [CRITICAL] Data — a normal pool imported from another source is admitted by a transfer probe, never by similarity
**Triggers:** negative pool, normal bags, imported normals, cross-dataset, D2City, BDD-A, BDDA, 0_Normal_Driving, BDD100K, foreign corpus, source shortcut, shortcut AUC, domain gap, negative bags, corpus build, MIL negatives, G-X
**Problem:** Three times now a pool of normal clips from a *different source* than the positives has been proposed as MIL negatives because it looked alike: `0_Normal_Driving` (same dataset, dashcam, fps — and it leaked through length, C28/C32), TAD's normals (the model collapsed into a clip classifier, C14), and D2City (same country, dashcam, 25 vs 30 fps). Measured 2026-09-25 on frozen CLIP, with length already matched (G-L 0.5009): a probe with D2City as its **only** negatives localizes accidents inside DADA videos at `auc_macro` **0.5864** vs **0.6763** for the videos' own normal frames — Δ **−0.0899**, t95 [−0.1011, −0.0787], 5/5 folds — and ranks every DADA normal frame above every D2City frame (shortcut AUC **1.000**). Adding D2City beside the in-video negatives still costs **−0.0123** (5/5 folds < 0, shortcut 0.999). The source is visible before any training: a probe that never saw D2City already puts it below DADA normals in **82 %** of pairs, and DADA-vs-DoTA separates at **0.9999** — every foreign dashcam corpus is linearly separable from DADA on frozen CLIP, so a crop (the 2.40:1 band arm moved X by −0.010) cannot remove the shortcut. Nothing raises: the bags load, MIL trains, and "normal" bags become trivially normal by camera. **Fourth failure, 2026-09-26 — BDD-A** (US, braking-enriched; gate read on the calm arm, no overlay): X **0.5492**, Δ(X−R0) **−0.1270** [−0.1532, −0.1008], 5/5 folds, shortcut **1.000** — worse than D2City. R0 started on the *opposite* side (BDD-A ranked above DADA normals in 63 % of pairs, shortcut 0.374), so the direction of the prior gap does not matter: a separable pool ends at shortcut 1.0. G-M passed only at ≈ 0 (−0.003), i.e. no gain.
...
**Files:** colab/D2City/eda_normal_bags.ipynb (§4), core/docs/D2CITY_EDA.md, .project/plans/katvad-d2city-normal-bag-eda.md §6 P5, outputs/EDA/D2City/, colab/BDDA/eda_normal_bags.ipynb, core/docs/BDDA_EDA.md, .project/plans/katvad-bdda-normal-bag-eda.md App. A, outputs/EDA/BDDA/
