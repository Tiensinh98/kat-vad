...
| `core/docs/gate/RESULTS_DADA.md` (moved from `v3/` in `cc39882`) | **The DADA-2000 campaign** (2026-09-06): the ordering inverts, and the in-domain 0.86 is a clip-level number. | Before quoting any DADA number |
...
| `core/docs/{TAD,DADA,PREVAD}_SETUP.md` + `core/docs/EDA.md` + `core/docs/gate/setup/*.md` | Per-corpus data and arm runbooks. On `main` only `gate/setup/{DADA,TAD}_V3_SETUP.md` exist (moved from `v3/setup/` in `cc39882`), and **their arm matrices prescribe `kip.gate_type`, which `main` cannot parse** — run those arms on `v3`. | Before running a campaign |
| **`core/docs/v2/KAT_VAD_PROPOSAL_v2.md`** + **`core/docs/v2/KAT_VAD_v2_ARCHITECTURE.md`** | **KAT-VAD v2 (2026-09-27, advisor sign-off; NOT built).** KIP + RAFT removed; frozen VideoMAE V2 motion stream (zero-init residual, fixed `σ_u`, `c`) + Clip-Referenced Normalization; rate-matched DoTA eval; losses unchanged; 2×2 factorial × 5 seeds; adoption rules vs the best adoptable free arm `F`. Four review-response tables. *(Not the old v2 of the gate line — that one never existed here.)* | **Before any v2 work, and before E0–E3** |
| `core/docs/REPORT_T2_OPTICAL_FLOW_FOR_ADVISOR.md` | The T2 / optical-flow report that motivated v2 (2026-09-26). | Before quoting the v2 motivation |
...
| ~~`.project/plans/katvad-v2-next-steps.md`~~ | **Does not exist in this tree** (unrelated to the 2026-09-27 `core/docs/v2/` architecture docs). The old v2 attribution tiers survive only as prose in `activeContext.md`. Do not cite it. | — |
...
## 14.5 Where the project stands (2026-09-27 — branch `main` = KAT-VAD **v1** code; **v2 designed, not built**)

**Baseline of truth = commit `b9978ff`** ("feat: Include training/testing for MSAD full"). Treat it as the known-good state; change it deliberately, never incidentally. **This branch is `main`, tip `cc39882` — the v1 line.** The v3 gate rebuild lives on branch `v3` (tip `bb1516c`); the two diverged at `fac71a3`. See §14.0.1.

- **Code on `main` (v1):** Phases 0–6 complete + DoTA zero-shot adapter, the offline metrics/rescore layer, the PreVAD preprocessor, the **TAD** (2026-09-02) and **DADA-2000** (2026-09-03/04) adapters, and the **`core/eda/` pre-flight profiler** (2026-09-06). KIP is the **v1** module: `PMGFlowHead` → `KinematicShift` (frozen 321-param MLP gate) → `MotionScoreHead`. Plus the **DADA-2000 original** adapter (`core/data/dada_origin.py`, 2026-09-15), the **`core/tools/grad_probe.py`** gradient-attribution CLI (2026-09-18) and the **`core/flow/zscore{,_cache}.py`** z-scored flow-target builder (2026-09-23). Plus `core/tools/subset_train.py` (2026-09-26). **91 Python files (61 source + 30 test), 14,155 source LOC, 30 docs** (measured 2026-09-27), CPU, data-free.
...
- **2026-09-26 — T2 learning curve (phase 6) RUN: INCONCLUSIVE → CCD parked → write-up (D)** (`RESULTS_DADA_ORIG_T2.md` §8; plan `katvad-t2-learning-curve.md` App. A). New tool `core/tools/subset_train.py`. T2 micro 25/50/100 % = 0.5947 / 0.6083 / 0.6182 (≈ +0.01 per doubling, decelerating). **DoTA flat over 4× data** (+0.008). The plan's power estimate was 4.5× too narrow, so FLAT was unreachable (pending (z)). Levers left: backbone probe (`DIAGNOSIS_DADA_FRAME_LEVEL_COLLAPSE.md` §7.4) and supervision — not KIP-v1, seeds or more T2-like data.
...
- **2026-09-27 — KAT-VAD v2 DESIGNED, advisor sign-off, NOT built** (`core/docs/v2/`). KIP/RAFT retired from the design; motion enters as an **input** (frozen VideoMAE V2, causal 1.5 s, squash) through `h = x̃ + W_u·(c·ũ⊘σ_u)`, `W_u` zero-init, **no per-token LayerNorm** (it would erase CRN's deviation magnitude, which reaches `V^t` via the encoder's outer residual `temporal_encoder.py:287`; CoAttn re-normalizes `V^u`). CRN reference chosen in E2 with a position ruler and a normals-only position fit. One decision interval (paired t95, 5 seeds, DoTA-dev); DoTA-dev/eval **50/50 fixed**; costly arms must beat the best adoptable free arm `F`. **Next: freeze + commit splits → E0 → E0b+E1 (one harness, DoTA-dev only) → E2(a)→(b)→(d); VideoMAE extraction in parallel.** No plan file yet (`.project/plans/katvad-v2-e0-e2.md`).
- **Tracks.** (0′) **KAT-VAD v2 E0–E2** — the next live work (bullet above). (0) **DADA-2000 original / T2 + Option A** — closed as a bounded null; T2 stays the training corpus. (0b) ~~**D2City negative pool**~~ — **closed 2026-09-25, NO-GO** (C38); ~~**BDD-A**~~ — **closed 2026-09-26, NO-GO** (C38). (1) **TAD** — campaign run 2026-09-14/15, collapsed (C14); the KIP A/B is blocked; next would be the destruction curve. (2) **DADA-2000 archive** — root-caused 2026-09-08 (length leak C28, whole-anchor DVS label C29) and **superseded as a training corpus** by the original release. (3) **PreVAD trunk transfer** — Gate P0 still unrun. **PreVAD can never host a KIP-on arm.**
- **Evidence, in measurement order:** `RESULTS_MSAD` → `RESULTS_DOTA` → `RESULTS_NCC` → `RESULTS_PHASE_A` → `RESULTS_ARM4_PROBE` → `RESULTS_PREVAD` → **`v3/RESULTS_V3_GATE_ATTRIBUTION`** (branch `v3` only) → **`gate/RESULTS_DADA`**, plus `REPORT_KIP_MSAD_DOTA_PREVAD.md` (and, on branch `v3`, the spec trio). **`outputs/` is gitignored**, so those documents are the only durable record — and on `main`, where `RESULTS_V3_GATE_ATTRIBUTION.md` is absent, `.project/memory-bank/{progress,activeContext}.md` is that record; the **62,254** per-clip `.npz` score files live on the user's disk/Drive.
...
- **A runbook on `main` is not runnable on `main` just because the file is there.** `core/docs/gate/setup/{DADA,TAD}_V3_SETUP.md` prescribe six arms keyed on `kip.gate_type`; this branch parses none of them. Check the flag exists in `core/config.py` before copy-pasting a runbook command.
