...
| **`.project/plans/katvad-flow-zscore-option-a.md`** + **`colab/DADA2000Origin/phase_5_zscore.ipynb`** | **THE LIVE PLAN (2026-09-23).** Option A: `flow/v2_zscore`, the pre-registered build gates (§6.1), the read-out table (§6.3), the runbook. P1 built (`ae4fded`); P0/P2–P4 not run. | **Start here for "what next"** |
| **`.project/plans/katvad-d2city-normal-bag-eda.md`** + **`colab/D2City/eda_normal_bags.ipynb`** | **NEW 2026-09-24 — D2City as an imported negative-bag pool** beside full-length DADA-original videos. Pre-registered gates G-I, G-L, R0 sanity, **G-X** (transfer probe — the admission gate), G-M, G-S (claim gate). Planned + dry-run; **not run**. | Before any cross-dataset normal pool, or before touching `data/D2City/` |
...
- **An imported normal pool is admitted by a transfer probe, not by similarity.** "Same country, dashcam, similar fps" held for `0_Normal_Driving` too, and it leaked (C28/C32). CLIP separates almost any two corpora, so the source probe (≥ 0.90 ⇒ no in-domain claim) is a *claim* gate; admission = G-X. Match the foreign pool's bag lengths by **drawing all lengths first, then packing** — per-clip drawing is biased short (pending (v)).
