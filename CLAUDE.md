...
| --- | --- | --- | --- |
| **`main` (tip `6a5f648`)** | **KAT-VAD v1** — the original KIP: `PMGFlowHead` + `KinematicShift` (frozen 321-param MLP gate) + `MotionScoreHead`. Plus the DoTA / PreVAD / TAD / DADA adapters and `core/eda/`. | **v1 only.** No `kip.gate_type`, no `core/kip/ecmr.py`, no gate diagnostics, no train-only inference graph. | **418 collected, 418 pass** (green since 2026-09-08) |
