...
| **CRITICAL** | Clip length alone predicts the label (lesson C28) | A constant-score-per-clip detector reading only the clip's frame count scores clip-level AUC 0.8105 and micro AUC 0.8654 — shorter clips are the abnormal ones. 107 normal clips (3,157 frames) fall outside the abnormal length range entirely. | Rebuild the corpus into fixed-length windows with the anomaly at a random offset. Until then, print this baseline beside every micro AUC and treat any arm that fails to beat it as unmeasured. |
...
- clip-level AUC **0.8105** (**shorter** clips are the abnormal ones); micro AUC **0.8654**, micro AP **0.2630**, macro AUC **0.5000** by construction
- abnormal clip length T: median **7.0**, min 1.0, max **17.0**
- normal clip length T: median **19.0**, min 1.0, max 72.0
- normal clips outside the abnormal length range entirely: **107** (3,157 frames)
...
- 1,530 clips, 20,087 frames, **23 raw dims**; dead dims: 0; non-finite frames: 0
- 23 effective dimensions, frame-global, no spatial content. Flow is train-time only and the train split carries no frame labels, so no flow-versus-label correlation is computable.
