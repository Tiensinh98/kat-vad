...
def preprocess_frames(frames: np.ndarray, crop_size: int = constants.CROP_SIZE) -> Tensor:
    """``(L,H,W,3)`` uint8 RGB -> ``(L,3,crop,crop)`` float32, CLIP-normalized.

    Resize shorter side to ``crop_size`` (bilinear, antialiased) then center
    crop — the CLIPImageProcessor convention, re-implemented deterministically
    so tests need no downloaded processor config.
    """
...
def encode_video(
    ...
    pixels = preprocess_frames(frames, crop_size)
    chunks: list[Tensor] = []
...
def encode_frame_dir(
    ...
    for start in range(0, len(paths), batch_size):
        pixels = preprocess_frames(read_images(paths[start : start + batch_size]), crop_size)
...
def extract_frame_directory(
    ...
    for video_id, folder in pending:
        ...
        features = encode_frame_dir(
            folder, encoder, device, stride, batch_size, subdir=subdir
        )
...
def extract_directory(
    ...
    for video_id, video_path in pending:
        target = output_dir / f"{video_id}.npy"
        features = encode_video(video_path, encoder, device, stride, batch_size)
        save_array(target, features)
        written.append(target)
        LOGGER.info("Saved %s %s -- %s", target.name, features.shape, progress.step())
    LOGGER.info("Extracted %d new feature files into %s", len(written), output_dir)
